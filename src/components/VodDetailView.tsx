import { ArrowLeft, Play, Film, Star, Calendar, Clock, User, Clapperboard, Heart } from "lucide-react";
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }} className="space-y-6">
        ...
        <div className="relative flex flex-col md:flex-row gap-6 sm:gap-8 p-4 sm:p-6 md:p-10">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="flex-1 flex flex-col justify-center min-w-0">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <Button onClick={() => onPlay(item)} className="gap-2 glow-accent" size="lg" disabled={!item.streamUrl}>
              ...
              {item.trailerUrl && (
                <Button variant="outline" size="lg" className="gap-2" onClick={() => setShowTrailer(!showTrailer)}>
      ...
      {showTrailer && item.trailerUrl && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="rounded-xl overflow-hidden card-shadow">
          <div className="relative w-full aspect-video bg-black">
            <iframe src={item.trailerUrl} className="w-full h-full" allow="autoplay; encrypted-media" allowFullScreen title={`Trailer - ${item.name}`} />
          </div>
        </motion.div>
      )}
    </motion.div>
